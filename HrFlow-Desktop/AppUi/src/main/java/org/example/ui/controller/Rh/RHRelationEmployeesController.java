...
public class RHRelationEmployeesController {
    ...
    @FXML private Label lblHeroTotalRequests;
    @FXML private Label lblHeroPendingRequests;
    @FXML private Label lblHeroApprovedRequests;
    ...
    private void loadAllDataAsync() {
        Task<Void> task = new Task<Void>() {
            @Override
            protected Void call() {
                List<Request> requests = requestService.getByRhId(rhId);
                List<RequestType> requestTypes = requestTypeService.getAll();
                List<Feedback> feedbacks = feedbackService.getByRhId(rhId);
                List<FeedbackFormation> ffs = ffService.getByRhId(rhId);
                List<Notification> notifs = notificationService.getByUserId(rhId);
                javafx.application.Platform.runLater(() -> {
                    requestsList.setAll(requests);
                    requestTypesList.setAll(requestTypes);
                    feedbacksList.setAll(feedbacks);
                    ffList.setAll(ffs);
                    ...
                    if (filterFormationFF != null) {
                        ...
                        ffs.stream()
                           ...
                           .distinct()
                           .forEach(names::add);
                        filterFormationFF.setItems(FXCollections.observableArrayList(names));
                        if (filterFormationFF.getValue() == null) filterFormationFF.setValue("Toutes");
                    }
                    applyFFFilters();
                    updateFFStats();
                });
        ...
        Thread t = new Thread(task, "relations-loader");
        t.setDaemon(true);
        t.start();
    }
    ...
    private void setupRequestsTable() {
        colRequestId.setCellValueFactory(cell ->
                new ReadOnlyObjectWrapper<>(cell.getValue() != null ? cell.getValue().getId() : null));
        colRequestId.setVisible(false);
    ...
    private void setupRequestTypesTable() {
        colTypeId.setCellValueFactory(cell ->
                new ReadOnlyObjectWrapper<>(cell.getValue() != null ? cell.getValue().getId() : null));
    ...
    private void setupFeedbacksTable() {
        colFeedbackId.setCellValueFactory(cell ->
                new ReadOnlyObjectWrapper<>(cell.getValue() != null ? cell.getValue().getId() : null));
    ...
    private void setupFFTable() {
        colFFId.setCellValueFactory(cell ->
                new ReadOnlyObjectWrapper<>(cell.getValue() != null ? cell.getValue().getId() : null));
        colFFId.setVisible(false);
    ...
    private void applyRequestFilters() {
        ...
        List<Request> allRequests = new ArrayList<>(requestsList);
    ...
    private void applyFeedbackFilters() {
        ...
        List<Feedback> allFeedbacks = new ArrayList<>(feedbacksList);
    ...
    private void applyFFFilters() {
        ...
        List<FeedbackFormation> all = new ArrayList<>(ffList);
    ...
    private void updateRequestStats() {
        ...
        lblPendingRequests.setText(String.valueOf(pending));
        lblApprovedRequests.setText(String.valueOf(approved));
        lblRejectedRequests.setText(String.valueOf(rejected));

        if (lblHeroTotalRequests != null) {
            lblHeroTotalRequests.setText(String.valueOf(total));
        }
        if (lblHeroPendingRequests != null) {
            lblHeroPendingRequests.setText(String.valueOf(pending));
        }
        if (lblHeroApprovedRequests != null) {
            lblHeroApprovedRequests.setText(String.valueOf(approved));
        }
    }
    ...
    private void updateFeedbackStats() {
        ...
        lblTotalFeedbacks.setText(String.valueOf(total));
    ...
    private void updateFFStats() {
        ...
        lblTotalFF.setText(String.valueOf(total));
        lblAvgRatingFF.setText(String.format("%.1f", avg));
        lblRecommandationFF.setText(String.format("%.0f%%", rate));
    }
