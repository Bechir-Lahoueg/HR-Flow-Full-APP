...
import org.example.ui.controller.Admin.AdminDashboardController;
...
public class LoginController {
    ...
    private void handleUserLogin() {
        ...
        userErrorLabel.setText("⏳ Connexion en cours...");
        userErrorLabel.setStyle("-fx-text-fill: #6b7280;");
        userUsernameField.setDisable(true);
        userPasswordField.setDisable(true);

        Task<User> loginTask = new Task<User>() {
            @Override
            protected User call() {
                return userController.handleLogin(username, password);
            }
        };
        loginTask.setOnSucceeded(e -> {
            userUsernameField.setDisable(false);
        ...
        loginTask.setOnFailed(e -> {
            userUsernameField.setDisable(false);
            userPasswordField.setDisable(false);
            userErrorLabel.setText("❌ Erreur de connexion. Réessayez.");
            userErrorLabel.setStyle("-fx-text-fill: #dc3545; -fx-font-weight: bold;");
        });
        Thread t = new Thread(loginTask, "login-user");
        t.setDaemon(true);
        t.start();
    }
    ...
    private void handleEmployeeLogin() {
        ...
        employeeErrorLabel.setText("⏳ Connexion en cours...");
        employeeErrorLabel.setStyle("-fx-text-fill: #6b7280;");
        employeeEmailField.setDisable(true);
        employeePasswordField.setDisable(true);

        Task<Employee> loginTask = new Task<Employee>() {
            @Override
            protected Employee call() {
                return employeeController.handleEmployeeLogin(email, password);
            }
        };
        loginTask.setOnSucceeded(e -> {
            employeeEmailField.setDisable(false);
            employeePasswordField.setDisable(false);
            Employee employee = loginTask.getValue();
            if (employee != null) {
                employeeErrorLabel.setText("");
        ...
        loginTask.setOnFailed(e -> {
            employeeEmailField.setDisable(false);
            employeePasswordField.setDisable(false);
            employeeErrorLabel.setText("❌ Erreur de connexion. Réessayez.");
            employeeErrorLabel.setStyle("-fx-text-fill: #dc3545; -fx-font-weight: bold;");
        });
        Thread t = new Thread(loginTask, "login-employee");
        t.setDaemon(true);
        t.start();
    }
