...
public class EmployeeFormationController {
    ...
    private void updateNavigationButtonStyles(String activePanel) {
        String inactiveStyle = "-fx-background-color: transparent; -fx-text-fill: #a0aec0; -fx-padding: 16 20; " +
                "-fx-cursor: hand; -fx-font-weight: bold; -fx-font-size: 13px; " +
                "-fx-border-width: 0 0 3 0; -fx-border-color: transparent; -fx-border-radius: 0; -fx-effect: null;";
        String activeStyle = "-fx-background-color: transparent; -fx-text-fill: #667eea; -fx-padding: 16 20; " +
                "-fx-cursor: hand; -fx-font-weight: bold; -fx-font-size: 13px; " +
                "-fx-border-width: 0 0 3 0; -fx-border-color: #667eea; -fx-border-radius: 0; -fx-effect: null;";

        if (btnTabFormations != null) btnTabFormations.setStyle(inactiveStyle);
        if (btnTabSessions != null) btnTabSessions.setStyle(inactiveStyle);
        if (btnTabInscriptions != null) btnTabInscriptions.setStyle(inactiveStyle);
    ...
    private VBox createSessionCard(SessionFormation session) {
        ...
        card.setStyle("-fx-background-color: white; -fx-border-color: #D0D0D0; -fx-border-radius: 8; " +
                "-fx-background-radius: 8; -fx-border-width: 1; -fx-effect: dropshadow(three-pass-box, rgba(0,0,0,0.08), 6, 0, 0, 2);");
    ...
    private void setupInscriptionsTable() {
        if (searchInscriptions != null) {
            searchInscriptions.textProperty().addListener((obs, oldVal, newVal) -> displayInscriptions(newVal));
        }
    ...
    private void loadSessionsForFormation(Integer idFormation) {
        ...
        try {
            ...
            for (SessionFormation session : allSessions) {
                if (idFormation != null && session.getIdFormation() != idFormation) continue;
                ...
                if (belongsToEmployeeRh) {
                    vboxSessions.getChildren().add(createSessionCard(session));
                    sessionCount++;
                }
            }

            if (sessionCount == 0) displayNoSessionsMessage();
    ...
    private void handleInscriptionForCard(SessionFormation session) {
        try {
            if (isAlreadyRegistered(session.getIdSession())) {
                showAlert(Alert.AlertType.WARNING, "Déjà inscrit", "Vous êtes déjà inscrit à cette session.");
                return;
            }
                ...
                if (response == ButtonType.OK) {
                    try {
                        ParticipationFormation participation = new ParticipationFormation(
                                ...
                                "En attente",                // ✅ valeur correcte pour la BD
                                currentEmployee.getId(),     // idEmployee
                                session.getIdSession(),      // idSession
                                ""                           // token
                        );
                        ...
                        showAlert(Alert.AlertType.INFORMATION, "Succès", "Votre inscription a été enregistrée avec succès !");
            ...
            showAlert(Alert.AlertType.ERROR, "Erreur", "Impossible de s'inscrire : " + ex.getMessage());
        }
    }
    ...
    private void showAlert(Alert.AlertType type, String title, String content) {
        ...
        String iconEmoji;
        switch (type) {
            case INFORMATION -> { headerColor = "#3FA9F5"; iconEmoji = "ℹ️"; }
            case WARNING     -> { headerColor = "#f39c12"; iconEmoji = "⚠️"; }
            case ERROR       -> { headerColor = "#e74c3c"; iconEmoji = "❌"; }
            default          -> { headerColor = "#27ae60"; iconEmoji = "❓"; }
        }
    ...
    private void displayInscriptions(String searchText) {
        ...
        try {
            ...
            System.out.println("=== DEBUG INSCRIPTIONS ===");
            System.out.println("Employee ID: " + currentEmployee.getId());
            System.out.println("Total inscriptionsList: " + inscriptionsList.size());
            for (ParticipationFormation p : inscriptionsList) {
                System.out.println("  -> idParticipation=" + p.getIdParticipation()
                        + " | idEmployee=" + p.getIdEmployee()
                        + " | idSession=" + p.getIdSession()
                        + " | statut='" + p.getStatutParticipation() + "'");
            }
            System.out.println("employeeParticipations count: " + employeeParticipations.size());


            java.util.Map<Formation, List<ParticipationFormation>> participationsByFormation = new java.util.LinkedHashMap<>();
            ...
            for (ParticipationFormation participation : employeeParticipations) {
                ...
                for (SessionFormation s : sessionService.getAllSessions()) {
                    if (Integer.valueOf(s.getIdSession()).equals(participation.getIdSession())) {
                        session = s;
                        break;
                    }
                }
                ...
                if (session != null) {
                    ...
                    for (Formation f : allFormations) {
                        if (Integer.valueOf(f.getIdFormation()).equals(session.getIdFormation())) {
                            formation = f;
                            break;
                        }
                    }
                    ...
                    if (formation != null) {
                        if (searchText == null || searchText.isEmpty() ||
                            ...
                            participationsByFormation
                                    .computeIfAbsent(formation, k -> new java.util.ArrayList<>())
                                    .add(participation);
                        }
                    }
                }
            ...
            for (Formation formation : participationsByFormation.keySet()) {
                vboxInscriptions.getChildren().add(
                        createInscriptionFormationCard(formation, participationsByFormation.get(formation))
                );
            }
    ...
    private VBox createInscriptionFormationCard(Formation formation, List<ParticipationFormation> participations) {
        ...
        mainCard.setStyle("-fx-background-color: white; -fx-border-color: #D0D0D0; -fx-border-radius: 10; " +
                "-fx-background-radius: 10; -fx-border-width: 1; -fx-effect: dropshadow(three-pass-box, rgba(0,0,0,0.08), 6, 0, 0, 2);");
        ...
        // --- BADGES STATUT (avec les vraies valeurs de la BD) ---
        HBox statsBox = new HBox(8);
        statsBox.setAlignment(Pos.CENTER_LEFT);
        statsBox.setPadding(new Insets(5, 0, 0, 0));

        long pendingCount  = participations.stream().filter(p -> "En attente".equalsIgnoreCase(p.getStatutParticipation())).count();
        long approvedCount = participations.stream().filter(p -> "Accepte".equalsIgnoreCase(p.getStatutParticipation())).count();
        long rejectedCount = participations.stream().filter(p -> "Refuse".equalsIgnoreCase(p.getStatutParticipation())).count();

        if (pendingCount > 0) {
            Label l = new Label("⏳ " + pendingCount + " en attente");
            l.setStyle("-fx-background-color: #fef3c7; -fx-text-fill: #92400e; -fx-background-radius: 12; -fx-padding: 3 8; -fx-font-size: 11px; -fx-font-weight: bold;");
            statsBox.getChildren().add(l);
        }
        if (approvedCount > 0) {
            Label l = new Label("✅ " + approvedCount + " acceptée(s)");
            l.setStyle("-fx-background-color: #dcfce7; -fx-text-fill: #166534; -fx-background-radius: 12; -fx-padding: 3 8; -fx-font-size: 11px; -fx-font-weight: bold;");
            statsBox.getChildren().add(l);
        }
        if (rejectedCount > 0) {
            Label l = new Label("❌ " + rejectedCount + " refusée(s)");
            l.setStyle("-fx-background-color: #fee2e2; -fx-text-fill: #991b1b; -fx-background-radius: 12; -fx-padding: 3 8; -fx-font-size: 11px; -fx-font-weight: bold;");
            statsBox.getChildren().add(l);
        }

        header.getChildren().addAll(titleLabel, typeLabel, statsBox);
        ...
        List<ParticipationFormation> pendingParticipations = participations.stream()
                .filter(p -> "En attente".equalsIgnoreCase(p.getStatutParticipation())).toList();

        List<ParticipationFormation> approvedParticipations = participations.stream()
                .filter(p -> "Accepte".equalsIgnoreCase(p.getStatutParticipation())).toList();

        List<ParticipationFormation> rejectedParticipations = participations.stream()
                .filter(p -> "Refuse".equalsIgnoreCase(p.getStatutParticipation())).toList();
        ...
        if (!pendingParticipations.isEmpty()) {
            ...
            for (ParticipationFormation p : pendingParticipations) {
                pendingSection.getChildren().add(createParticipationCard(p, "pending"));
            }
        ...
        if (!approvedParticipations.isEmpty()) {
            ...
            for (ParticipationFormation p : approvedParticipations) {
                approvedSection.getChildren().add(createParticipationCard(p, "approved"));
            }
        ...
        if (!rejectedParticipations.isEmpty()) {
            ...
            for (ParticipationFormation p : rejectedParticipations) {
                rejectedSection.getChildren().add(createParticipationCard(p, "rejected"));
            }
    ...
    private VBox createParticipationCard(ParticipationFormation participation, String status) {
        ...
        sessionLabel.setStyle("-fx-font-size: 12px; -fx-text-fill: #34495e;");
        ...
        // ✅ Badge statut — une seule déclaration propre
        String bgColor;
        String statusLabel;
        String textColor = "white";

        switch (status) {
            case "approved" -> { bgColor = "#27ae60"; statusLabel = "✅ Acceptée"; }
            case "rejected" -> { bgColor = "#e74c3c"; statusLabel = "❌ Refusée"; }
            default         -> { bgColor = "#f39c12"; statusLabel = "⏳ En attente"; }
        }
        ...
        // Bouton certificat — uniquement si Accepte ET session terminée
        if ("approved".equals(status)) {
            ...
            if (session != null) {
                ...
                if (!today.isBefore(session.getDateFin())) {
                    Button btnDownloadCert = new Button("🎓 Télécharger Certificat");
                ...
                confirm.showAndWait().ifPresent(response -> {
                    if (response == ButtonType.OK) {
                        try {
                            ...
                            loadInscriptions();
                            displayInscriptions(searchInscriptions != null ? searchInscriptions.getText() : "");
    ...
    private void handleViewInscriptionDetails(ParticipationFormation selected) {
        ...
        // ✅ Couleurs avec les vraies valeurs de la BD
        String statusColor = "#3FA9F5";
        if ("Accepte".equalsIgnoreCase(selected.getStatutParticipation())) {
            statusColor = "#27ae60";
        } else if ("Refuse".equalsIgnoreCase(selected.getStatutParticipation())) {
            statusColor = "#e74c3c";
        } else if ("En attente".equalsIgnoreCase(selected.getStatutParticipation())) {
            statusColor = "#f39c12";
        }
        ...
        contentBox.getChildren().addAll(
                lblFormationTitle, lblFormationValue,
                lblSessionTitle, lblSessionValue,
                lblDateTitle, lblDateValue,
                lblStatutTitle, lblStatutValue
        );
        ...
        if (selected.getResultat() != null && !selected.getResultat().isEmpty()) {
            ...
            contentBox.getChildren().addAll(lblResultatTitle, lblResultatValue);
        }
    ...
    private void handleAfficherQRCode(Formation f) {
        ...
        task.setOnFailed(ev -> showAlert(Alert.AlertType.ERROR, "Erreur API", "La génération du QR Code a échoué."));
    ...
    private void handleEcouterFormation(Formation f) {
        try {
            ...
            task.setOnFailed(ev -> showAlert(Alert.AlertType.ERROR, "Erreur TTS",
                    "Impossible de générer l'audio : " + task.getException().getMessage()));
