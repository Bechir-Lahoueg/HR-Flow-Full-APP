...
public class RHProjectController {
    ...
    private void loadProjectsAsync() {
        tableProjects.setPlaceholder(new javafx.scene.control.ProgressIndicator());

        Task<List<Project>> task = new Task<List<Project>>() {
            @Override
            protected List<Project> call() {
                return projectService.getByRhId(rhId);
            }
        };

        task.setOnSucceeded(e -> {
            projectsList.setAll(task.getValue());
            tableProjects.setPlaceholder(new javafx.scene.control.Label("Aucun projet"));
            updateStatistics();
        });

        task.setOnFailed(e -> tableProjects.setPlaceholder(
            new javafx.scene.control.Label("Erreur de chargement")));

        Thread t = new Thread(task, "projects-loader");
        t.setDaemon(true);
        t.start();
    }
    ...
    private void loadProjects() {
        projectsList.setAll(projectService.getByRhId(rhId));
        applyFilters();
    }
    ...
    private void applyFilters() {
        ...
        List<Project> all = new java.util.ArrayList<>(projectsList);
    ...
    private void handleViewDetails() {
        ...
        try {
            java.net.URL detailsUrl = MainApp.class.getResource(
                    "/fxml/views/Rh-dashboard/RHProjectDetailsView.fxml");
            if (detailsUrl == null) {
                showAlert(Alert.AlertType.ERROR, "Erreur",
                        "Vue de détails introuvable: RHProjectDetailsView.fxml");
                return;
            }

            FXMLLoader loader = new FXMLLoader(detailsUrl);
            Parent root = loader.load();

            javafx.stage.Stage stage = MainApp.getPrimaryStage();
            if (stage != null) {
                javafx.scene.Scene scene = new javafx.scene.Scene(root, 1280, 800);
                java.net.URL cssUrl = MainApp.class.getResource("/css/style.css");
                if (cssUrl != null) {
                    scene.getStylesheets().add(cssUrl.toExternalForm());
                }
                stage.setScene(scene);
                stage.setTitle("Détails Projet - " + selected.getName());
                ...
                if (scene != null) {
                    ...
                    showAlert(Alert.AlertType.ERROR, "Erreur",
                            "Impossible d'afficher la vue de détails (Scene introuvable)."
                    );
                    return;
                }
            }
            ...
            if (controller != null) {
                try {
                    ...
                    showAlert(Alert.AlertType.WARNING, "Attention",
                            "La vue de détails est affichée, mais certaines données n'ont pas pu être chargées.");
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
            ...
            showAlert(Alert.AlertType.ERROR, "Erreur",
                    "Impossible de charger la vue de détails du projet: " + msg);
        }
    }
    @FXML
