...
public class EmployeeRemunerationController {
    ...
    private void loadAllDataAsync() {
        ...
        Task<Object[]> task = new Task<Object[]>() {
            ...
            protected Object[] call() throws Exception {
                ...
                List<FichePaie> fiches = fps.getFichesByEmploye(currentEmployee.getId());
                List<Prime> primes = ps.getPrimesByEmployee(currentEmployee.getId());
                List<Deduction> deductions = ds.getDeductionsByEmployee(currentEmployee.getId());
                return new Object[]{conn, fps, ps, ds, fiches, primes, deductions};
            }
        };
        ...
        task.setOnSucceeded(e -> {
            ...
            fichePaieService = (FichePaieService) data[1];
            primeService = (PrimeService) data[2];
            deductionService = (DeductionService) data[3];
            @SuppressWarnings("unchecked") List<FichePaie> fiches = (List<FichePaie>) data[4];
            @SuppressWarnings("unchecked") List<Prime> primes = (List<Prime>) data[5];
            @SuppressWarnings("unchecked") List<Deduction> deductions = (List<Deduction>) data[6];
            fichesList.setAll(fiches);
            primesList.setAll(primes);
            deductionsList.setAll(deductions);
            if (lblPrimeCount != null) lblPrimeCount.setText(primes.size() + " prime(s)");
            if (lblDeductionCount != null) lblDeductionCount.setText(deductions.size() + " déduction(s)");
        ...
        task.setOnFailed(e -> {
            ...
            System.err.println("Erreur chargement rémunération employé: " + task.getException().getMessage());
        });

        Thread t = new Thread(task, "employee-remuneration-loader");
        t.setDaemon(true);
        t.start();
    }
    ...
    private void loadFiches() {
        ...
        try {
            List<FichePaie> employeeFiches = fichePaieService.getFichesByEmploye(currentEmployee.getId());
            fichesList.clear();
    ...
    private void filterFichesByYear(int year) {
        ...
        try {
            List<FichePaie> employeeFiches = fichePaieService.getFichesByEmploye(currentEmployee.getId());
            List<FichePaie> filtered = employeeFiches.stream()
                .filter(f -> f.getAnnee() == year)
    ...
    private void loadPrimes() {
        ...
        try {
            List<Prime> employeePrimes = primeService.getPrimesByEmployee(currentEmployee.getId());
    ...
    private void loadDeductions() {
        ...
        try {
            List<Deduction> employeeDeductions = deductionService.getDeductionsByEmployee(currentEmployee.getId());
    ...
    private void handleDownloadFiche() {
        ...
        fileChooser.setInitialFileName(
                "FichePaie_" + selected.getMoisNom() + "_" + selected.getAnnee() + ".pdf");
    ...
    private void showFicheDetails(FichePaie fiche) {
        ...
        alert.setHeaderText(fiche.getMoisNom() + " " + fiche.getAnnee());
