...
public class RHProjectDetailsController {
    ...
    public void initData(Project project, User user) {
        ...
        this.rhId = user != null ? user.getId() : project.getRhId();
    ...
    public void initDataForEmployee(Project project, Employee employee) {
        if (project == null) {
            showAlert(Alert.AlertType.ERROR, "Erreur", "Projet introuvable.");
            return;
        }
    ...
    private void loadProjectInfo() {
        Project refreshed = projectService.getById(currentProject.getId());
        if (refreshed != null) {
            currentProject = refreshed;
        }
        if (currentProject == null) {
            showAlert(Alert.AlertType.ERROR, "Erreur", "Impossible de charger les infos du projet.");
            return;
        }
...
// ═══════════════════════════════════════════════════════════════
    ...
    private void loadAnalytics() {
        if (lblPredictionDetail == null || lblHealthScoreDetail == null || vboxHealthDetails == null || vboxRisks == null) {
            return;
        }
    ...
    private void loadTeam() {
        teamList.clear();
        List<ProjectCollaborator> collaborators = collaboratorService.getByProjectId(currentProject.getId());
        if (collaborators != null) {
            teamList.addAll(collaborators);
        }

        int totalHours = teamList.stream()
                .map(ProjectCollaborator::getWorkedHours)
    ...
    private void loadMilestones() {
        ...
        List<ProjectMilestone> milestones = milestoneService.getByProjectId(currentProject.getId());
        if (milestones != null) {
        ...
        try {
            ...
            System.err.println("WARN: checkDelayed a echoue pour le projet " + currentProject.getId());
        }
    }
    ...
    private void updateKanbanBoard() {
        if (kanbanTodo == null || kanbanInProgress == null || kanbanReview == null || kanbanDone == null) {
            return;
        }
