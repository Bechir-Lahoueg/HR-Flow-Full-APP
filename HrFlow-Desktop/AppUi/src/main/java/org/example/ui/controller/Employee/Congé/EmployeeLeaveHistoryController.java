...
public class EmployeeLeaveHistoryController {
    ...
    /** Recharge l'historique depuis la base de données (en arrière-plan). */
    public void refresh() {
        Task<List<LeaveRequest>> task = new Task<>() {
            @Override
            protected List<LeaveRequest> call() {
                return leaveRequestService.getEmployeeLeaveRequests(currentEmployeeId);
            }
        };
        task.setOnSucceeded(e -> {
            leaveRequestsList.setAll(task.getValue());
        ...
        task.setOnFailed(e ->
                System.err.println("Erreur chargement historique: " + task.getException().getMessage()));
        new Thread(task, "employee-history-loader").start();
    }
