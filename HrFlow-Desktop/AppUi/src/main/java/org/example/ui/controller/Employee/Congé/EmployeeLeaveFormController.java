...
public class EmployeeLeaveFormController {
    ...
    @FXML private RadioButton exceptionModeRadio;
    @FXML private VBox exceptionPanel;
    @FXML private ComboBox<String> urgencyComboBox;
    @FXML private Button aiJustificationButton;
    ...
    public void initialize() {
        ...
        setupExceptionModeToggle();
        if (aiJustificationButton != null)
            aiJustificationButton.setOnAction(e -> generateAiJustification());
    ...
    private void setupExceptionModeToggle() {
        if (normalModeRadio == null || exceptionModeRadio == null) return;
        ToggleGroup group = new ToggleGroup();
        normalModeRadio.setToggleGroup(group);
        exceptionModeRadio.setToggleGroup(group);
        normalModeRadio.setSelected(true);
        if (urgencyComboBox != null)
            urgencyComboBox.setItems(FXCollections.observableArrayList("Faible (LOW)", "Moyen (MEDIUM)", "Haute (HIGH)"));
        showExceptionPanel(false);
        group.selectedToggleProperty().addListener((obs, o, n) -> {
            boolean isException = n == exceptionModeRadio;
            showExceptionPanel(isException);
            updateBalanceWarning();
        });
    }

    private void showExceptionPanel(boolean show) {
        if (exceptionPanel != null) {
            exceptionPanel.setVisible(show);
            exceptionPanel.setManaged(show);
        }
    }

    private boolean isExceptionMode() {
        return exceptionModeRadio != null && exceptionModeRadio.isSelected();
    }

    private String getSelectedUrgency() {
        if (urgencyComboBox == null || urgencyComboBox.getValue() == null) return null;
        String v = urgencyComboBox.getValue();
    ...
    private void updateBalanceWarning() {
        if (balanceWarningLabel == null) return;
        LeaveBalance balance = leaveBalanceService.getBalance(currentEmployeeId);
        double avail = (balance != null) ? balance.getAvailableDays() : 0;
        if (avail <= 0 && !isExceptionMode()) {
            balanceWarningLabel.setText("⚠️ Solde épuisé (" + String.format("%.1f", avail) + " j). Utilisez le mode Exceptionnel.");
            balanceWarningLabel.setVisible(true);
            balanceWarningLabel.setManaged(true);
        } else {
            balanceWarningLabel.setVisible(false);
            balanceWarningLabel.setManaged(false);
        }
    }

    /** Generates AI justification text and pastes it into the reason field. */
    private void generateAiJustification() {
        LocalDate start = startDatePicker.getValue();
        LocalDate end   = endDatePicker.getValue();
        String leaveType = leaveTypeComboBox.getValue();
        String urgency  = getSelectedUrgency();
        if (start == null || end == null) {
            showAlert(Alert.AlertType.WARNING, "IA", "Veuillez d'abord sélectionner les dates.");
            return;
        }
        if (aiJustificationButton != null) aiJustificationButton.setDisable(true);
        String reason = reasonTextArea.getText();
        String lt = (leaveType != null) ? leaveType : "Congé exceptionnel";
        String urg = (urgency != null) ? urgency : "MEDIUM";
        Task<String> task = new Task<>() {
            @Override protected String call() {
                return leaveAiService.generateEmployeeLeaveJustification(lt, start, end, urg, reason);
            }
        };
        task.setOnSucceeded(e -> {
            reasonTextArea.setText(task.getValue());
            if (aiJustificationButton != null) aiJustificationButton.setDisable(false);
        });
        task.setOnFailed(e -> {
            if (aiJustificationButton != null) aiJustificationButton.setDisable(false);
        });
    ...
    private void submitLeaveRequest() {
        ...
        confirmation.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                String mode   = isExceptionMode() ? LeaveRequestService.CATEGORY_EXCEPTION : LeaveRequestService.CATEGORY_NORMAL;
                String urgency = isExceptionMode() ? getSelectedUrgency() : null;
                ...
                if (result.isSuccess()) {
                    String modeLabel = isExceptionMode() ? " [EXCEPTIONNEL]" : "";
                    showAlert(Alert.AlertType.INFORMATION, "✅ Succès",
                            "Votre demande de congé" + modeLabel + " a été soumise avec succès!\n\n"
                            + result.getMessage() + "\n\n"
                            + (isExceptionMode()
                               ? "Elle sera examinée par votre RH, puis validée par l'Admin."
                               : "Elle sera examinée par le service RH dans les plus brefs délais."));
                    InAppNotificationService.getInstance().notifyAllRH(
                            currentEmployeeName + " a soumis une demande de congé" + modeLabel + " ("
    ...
    private boolean validateForm() {
        ...
        if (isExceptionMode()) {
            String urgency = getSelectedUrgency();
            if (urgency == null) {
                showAlert(Alert.AlertType.WARNING, "⚠️ Validation",
                    "Pour une demande exceptionnelle, veuillez sélectionner un niveau d'urgence.");
                return false;
            }
            String reason2 = reasonTextArea.getText();
            if (reason2 == null || reason2.trim().length() < 15) {
                showAlert(Alert.AlertType.WARNING, "⚠️ Validation",
                    "Pour une demande exceptionnelle, la justification doit comporter au moins 15 caractères.");
                return false;
            }
        }
        String reason = reasonTextArea.getText();
    ...
    private void clearForm() {
        ...
        if (normalModeRadio != null) normalModeRadio.setSelected(true);
        showExceptionPanel(false);
        if (urgencyComboBox != null) urgencyComboBox.getSelectionModel().clearSelection();
        if (balanceWarningLabel != null) { balanceWarningLabel.setVisible(false); balanceWarningLabel.setManaged(false); }
        daysCountLabel.setText("--");
        daysCountLabel.setStyle("-fx-font-size: 24px; -fx-text-fill: #667eea; -fx-font-weight: bold;");
        if (calendarDaysLabel != null) { calendarDaysLabel.setText("--"); calendarDaysLabel.setStyle("-fx-font-size: 24px; -fx-text-fill: #4a5568; -fx-font-weight: bold;"); }
        if (weekendDaysLabel  != null) { weekendDaysLabel.setText("--");  weekendDaysLabel.setStyle("-fx-font-size: 24px; -fx-text-fill: #718096; -fx-font-weight: bold;"); }
