...
public class EmployeeProjectController {
    ...
    private void setupProjectsTable() {
        ...
        //  Double-click pour ouvrir le Kanban
    ...
    private void openProjectKanban(Project project) {
        try {
            ...
            Employee employeeForKanban = currentEmployee;
            if (employeeForKanban == null) {
                employeeForKanban = new Employee();
                employeeForKanban.setId(currentEmployeeId);
            }
            controller.initData(project, employeeForKanban);

            // Remplacer la vue actuelle
            javafx.scene.Scene scene = backButton != null ? backButton.getScene() : null;
            if (scene == null && tableMyProjects != null) {
                scene = tableMyProjects.getScene();
            }
            if (scene == null) {
                throw new IllegalStateException("Scene introuvable pour la navigation.");
            }
            scene.setRoot(root);
