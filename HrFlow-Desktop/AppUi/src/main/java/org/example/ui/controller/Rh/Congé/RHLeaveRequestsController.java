...
public class RHLeaveRequestsController {
    ...
    @FXML private TableColumn<LeaveRequest, String>    categoryColumn;
    ...
    @FXML private Button manualCreditButton;
    ...
    /** Recharge les demandes des employés rattachés à ce RH (en arrière-plan). */
    public void refresh() {
        Task<List<LeaveRequest>> task = new Task<>() {
            @Override
            protected List<LeaveRequest> call() {
                return rhId > 0
                        ? leaveRequestService.getLeaveRequestsByRH(rhId)
                        : leaveRequestService.getAllLeaveRequests();
            }
        };
        task.setOnSucceeded(e -> {
            allRequests.setAll(task.getValue());
            applyFilter();
            if (onChangeCallback != null) onChangeCallback.run();
        });
        task.setOnFailed(e ->
                System.err.println("Erreur chargement demandes: " + task.getException().getMessage()));
        new Thread(task, "rh-leave-requests-loader").start();
    }
    ...
    private void setupTable() {
        ...
        // Category column (NORMAL/EXCEPTION badge)
        if (categoryColumn != null) {
            categoryColumn.setCellValueFactory(cd ->
                new javafx.beans.property.SimpleStringProperty(
                    LeaveRequest.CATEGORY_EXCEPTION.equals(cd.getValue().getRequestCategory())
                    ? "🚨 EXCEPTION" : "🟢 NORMAL"));
            categoryColumn.setCellFactory(col -> new TableCell<>() {
                @Override protected void updateItem(String item, boolean empty) {
                    ...
                    setText(item);
                    setStyle(item.contains("EXCEPTION")
                        ? "-fx-background-color: #fff5f5; -fx-text-fill: #c53030; -fx-font-weight: bold;"
                        : "-fx-background-color: #f0fff4; -fx-text-fill: #276749; -fx-font-weight: bold;");
                }
            });
        }
        // Workflow status column
        if (workflowColumn != null) {
            workflowColumn.setCellValueFactory(cd ->
                new javafx.beans.property.SimpleStringProperty(
                    cd.getValue().getWorkflowStatusLabel()));
        }
        ...
        leaveRequestsTable.setRowFactory(tv -> new TableRow<LeaveRequest>() {
            @Override protected void updateItem(LeaveRequest r, boolean empty) {
                super.updateItem(r, empty);
                if (empty || r == null) { setStyle(""); return; }
                if (LeaveRequest.CATEGORY_EXCEPTION.equals(r.getRequestCategory()))
    ...
    private void setupButtons() {
        ...
        if (manualCreditButton != null)
            manualCreditButton.setOnAction(e -> openManualCreditDialog());
    ...
    private void openManualCreditDialog() {
        // Load this RH's employees
        List<Employee> employees = new EmployeeController().handleListMyEmployees(currentRHUser);
        if (employees == null || employees.isEmpty()) {
            showAlert(Alert.AlertType.WARNING, "Crédit Manuel",
                    "Aucun employé trouvé pour votre compte.");
            return;
        }

        Dialog<ButtonType> dialog = new Dialog<>();
        dialog.setTitle("Crédit Manuel de Congé");
        dialog.setHeaderText("Accorder des jours de congé supplémentaires à un employé");

        GridPane grid = new GridPane();
        grid.setHgap(10); grid.setVgap(10);
        grid.setPadding(new javafx.geometry.Insets(20, 150, 10, 10));

        // Employee dropdown showing full name
        ComboBox<Employee> empCombo = new ComboBox<>(FXCollections.observableArrayList(employees));
        empCombo.setPromptText("Sélectionner un employé");
        empCombo.setPrefWidth(220);
        empCombo.setConverter(new StringConverter<Employee>() {
        ...
        daysField.setPromptText("ex: 2.5");

        grid.add(new Label("Employé :"), 0, 0);
        grid.add(empCombo, 1, 0);
        grid.add(new Label("Jours à ajouter :"), 0, 1);
        ...
        dialog.showAndWait().ifPresent(result -> {
            if (result == ButtonType.OK) {
                Employee selected = empCombo.getValue();
                if (selected == null) {
                    showAlert(Alert.AlertType.ERROR, "Erreur", "Veuillez sélectionner un employé.");
                    return;
                }
                try {
                    double days = Double.parseDouble(daysField.getText().trim());
                    String msg = leaveBalanceService.grantManualCredit(rhId, selected.getId(), days);
                    showAlert(Alert.AlertType.INFORMATION, "Crédit Manuel", msg);
                } catch (NumberFormatException ex) {
                    showAlert(Alert.AlertType.ERROR, "Erreur", "Veuillez entrer un nombre de jours valide (ex: 2.5).");
                }
            }
        });
    }
