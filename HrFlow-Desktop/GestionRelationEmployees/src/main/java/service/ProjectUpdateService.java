...
public class ProjectUpdateService {
    ...
    public boolean add(ProjectUpdate update) {
        String sql = "INSERT INTO project_updates " +
                "(project_id, user_id, update_type, title, content, created_at) " +
                "VALUES (?, ?, ?, ?, ?, ?)";
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                try {
                    ...
                    System.err.println("❌ Erreur mapRow ProjectUpdate : " + ex.getMessage());
                }
            }
        } catch (SQLException e) {
    ...
    private ProjectUpdate mapRow(ResultSet rs) throws SQLException {
        ...
        u.setUpdateType(parseUpdateType(rs.getString("update_type")));
    ...
    private ProjectUpdate.UpdateType parseUpdateType(String raw) {
        if (raw == null || raw.trim().isEmpty()) {
            return ProjectUpdate.UpdateType.comment;
        }
        try {
            return ProjectUpdate.UpdateType.valueOf(raw.trim());
