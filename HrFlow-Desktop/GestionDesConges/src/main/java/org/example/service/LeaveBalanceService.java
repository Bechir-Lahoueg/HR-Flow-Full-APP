...
public class LeaveBalanceService {
    ...
    /**
     * Octroie un crédit manuel de jours (RH → employé).
     * Mirrors web LeaveBalanceService::grantManualCredit().
     *
     * @param rhId       identifiant RH réalisant l'octroi
     * @param employeeId identifiant employé bénéficiaire
     * @param creditDays nombre de jours à ajouter (doit être > 0)
     * @return message de résultat (succès ou erreur)
     */
    public String grantManualCredit(int rhId, int employeeId, double creditDays) {
        if (creditDays <= 0)
            return "Le nombre de jours doit être supérieur à zéro.";
        // Validate that the employee belongs to this RH
        String checkSql = "SELECT COUNT(*) FROM users WHERE id=? AND rh_id=?";
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            if (!rs.next() || rs.getInt(1) == 0)
                return "Employé introuvable ou non rattaché à ce RH.";
        } catch (SQLException e) {
        ...
        LeaveBalance balance = findByEmployeeId(employeeId);
        if (balance == null)
            return "Solde inexistant pour cet employé (veuillez créer le solde d'abord).";

        double newAvailable = balance.getAvailableDays() + creditDays;
        double newAccrued   = balance.getTotalAccrued() + creditDays;
        String sql = "UPDATE leave_balance SET available_days=?, total_accrued=? WHERE employee_id=?";
        try (Connection conn = DatabaseConfig.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setDouble(1, newAvailable);
            ...
            if (ps.executeUpdate() > 0)
                return String.format("Crédit de %.1f jour(s) accordé avec succès. Nouveau solde: %.1f j.",
                        creditDays, newAvailable);
        } catch (SQLException e) {
            return "Erreur d'octroi: " + e.getMessage();
        }
        return "Erreur inconnue lors de l'octroi du crédit.";
    }
