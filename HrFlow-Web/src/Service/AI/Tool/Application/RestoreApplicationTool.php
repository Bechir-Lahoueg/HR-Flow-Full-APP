...
class RestoreApplicationTool implements ToolInterface
{
    ...
    public function getDefinition(): array
    {
        return [
            'name' => $this->getName(),
            'description' => "Restaure une candidature supprimée (soft delete) (propriétaire RH uniquement).",
            'parameters' => [
                'type' => 'object',
                'properties' => (object)[
    ...
    public function execute(array $args): mixed
    {
        ...
        $app = $this->applicationRepository->findOneByRhIncludingDeleted($id, $user);
        if (!$app) {
        ...
        return [
            'status' => 'success',
            'application_id' => $id,
            'message' => "✅ Candidature #{$id} restaurée.",
        ];
    }
}
